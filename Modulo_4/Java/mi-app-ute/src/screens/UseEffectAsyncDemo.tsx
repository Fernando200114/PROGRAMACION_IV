...
type TodoDTO = { id: number; nombre: string; pais:string };
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        const res:any = await fetch("https://llulluna-vuelos-api.desarrollo-software.xyz/api/aerolineas/");
    ...
    <View style={styles.container}>
      ...
      {!loading && !error ? (
        <View style={styles.list}>
          {items.map((t) => (
            <Text key={t.id} style={styles.item}>• {t.nombre +" "+ t.pais}</Text>
          ))}
        </View>
      ) : null}
    </View>
