...
export function FuerzaScreen() {
    ...
    function validate(next: TrapezoidForm): FormErrors {
        const e: FormErrors = {};
        const m = parseFloat(next.baseMayor);
        const a = parseFloat(next.baseMenor);

        if (!next.baseMayor || isNaN(m) || m <= 0) e.baseMayor = "Base mayor inválida";
        if (!next.baseMenor || isNaN(a) || a<= 0) e.baseMenor = "Base menor inválida";
    ...
    function submit() {
        ...
        const m = parseFloat(form.m);
        const a = parseFloat(form.a);
        
        const fuerza = (m *  a) 

        Alert.alert("📐 Resultado", `El área del trapecio es: ${fuerza.toFixed(2)} unidades²`);
    }
            ...
            <ScrollView contentContainerStyle={styles.container}>
                ...
                <View style={[styles.card, { backgroundColor: card, borderColor: border }]}>
                    ...
                    <Pressable
                        ...
                        onPress={submit}
                    >
                        <Text style={[styles.btnText, { color: primary }]}>Calcular Fuerza</Text>
                    </Pressable>
