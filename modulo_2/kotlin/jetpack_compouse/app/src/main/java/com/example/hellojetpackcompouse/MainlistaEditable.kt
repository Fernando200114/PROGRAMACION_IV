...
data class UsuarioList(val id: Int, val nombre: String, val rol: String)
...
fun ListaEditableScreen() {
    ...
    val usuarios = remember {
        mutableStateListOf(
            UsuarioList(1, "Ana Torres", "Diseñadora"),
            UsuarioList(2, "Luis Pérez", "Desarrollador"),
            UsuarioList(3, "María López", "Tester QA"),
            UsuarioList(4, "Carlos Ruiz", "Project Manager")
        )
    }
    ...
    // Estado del usuario seleccionado (debe tener tipo explícito)
    var seleccionado by remember { mutableStateOf<UsuarioList?>(null) }
    ...
    Column(
            ...
            Button(
                onClick = {
                    ...
                    if (n.isNotEmpty() && r.isNotEmpty()) {
                        autoId += 1
                        usuarios.add(UsuarioList(autoId, n, r))
                        nombre = ""
                        rol = ""
                    }
                },
        ...
        HorizontalDivider() // Reemplaza Divider() (deprecated)
